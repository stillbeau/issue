...
def load_google_sheet(creds_dict, spreadsheet_id, worksheet_name):
    ...
    try:
        ...
        critical_cols = ['Next Sched. - Activity', 'Next Sched. - Date', 'Next Sched. - Status', 'Install - Date', 'Supplied By'] 
        for col in critical_cols:
            if col not in df.columns:
                df[col] = "" # Or pd.NA if appropriate for the type before conversion
        return df, spreadsheet, gc 
    except gspread.exceptions.GSpreadException as e: # Catch gspread specific errors for duplicates
...
def preprocess_data(df):
    ...
    # Using column names with " - " as per user's latest list
    ...
    # Ensure this list only contains unique column names if there were any accidental duplicates in the definition
    unique_date_columns = []
...
def flag_keyword_issues(df):
    ...
    for col_name in actual_notes_cols_to_scan:
        clean_col_name_for_flag = re.sub(r'[^A-Za-z0-9_]+', '_', col_name) # Generates names like Next_Sched_Notes
...
def determine_primary_issue_and_days(row, current_calc_date_ts):
    ...
    # Flag column names are generated by replacing " - " with "_" and other non-alphanum with "_"
    # So, issue_desc_map keys need to match those generated flag names.
    # For keyword flags, the key will be like 'Flag_Keyword_In_Next_Sched_Notes'
    issue_desc_map = {
        'Flag_PastDue_Next_Sched_Activity': "Past Due: Next Sched. Activity", # Will be replaced dynamically
    ...
    for flag_col in flag_check_order:
        if row.get(flag_col, False): 
            ...
            if flag_col == 'Flag_PastDue_Next_Sched_Activity':
                # Use the actual activity from 'Next Sched. - Activity' column
...
if final_creds:
    if st.sidebar.button("🔄 Load and Analyze Job Data", key="load_analyze_button"):
        ...
        st.session_state.selected_next_activities = [] # Reset filters
...
if st.session_state.df_analyzed is not None and not st.session_state.df_analyzed.empty and 'Flag_Overall_Needs_Attention' in st.session_state.df_analyzed.columns:
    ...
    if not df_display_full.index.is_unique: # Ensure unique index for .loc operations if using original index
    ...
    if not priority_jobs_df_all_original.empty:
        # Apply determine_primary_issue_and_days to the original set of priority jobs
        ...
        if 'Next Sched. - Activity' in priority_jobs_df_all_original.columns:
            ...
            if unique_next_activities: # Only show multiselect if there are activities to filter by
                st.session_state.selected_next_activities = st.multiselect(
                    "Filter by Next Scheduled Activity:",
                    options=unique_next_activities,
                    default=st.session_state.selected_next_activities # Persist selection
                )
        
        # Apply Next Sched. - Activity filter
        ...
        # Filter out locally resolved/snoozed items for the current view AFTER other filters
        ...
        if not visible_priority_jobs_df.empty:
            ...
            if st.session_state.current_page < 0: # Should not happen if logic is correct
                 st.session_state.current_page = 0
            ...
            for job_index, row_data in jobs_to_display_on_page.iterrows(): 
                ...
                item_cols = st.columns([3, 1, 2, 2]) # Adjusted columns for Next Sched Activity
                ...
                item_cols[3].markdown(f"**Next Sched:** {row_data.get('Next Sched. - Activity', 'N/A')}")
            ...
            if st.button("✍️ Update 'todo' Sheet (with current filtered & paged view)", key="update_todo_sheet_button_interactive"):
                if st.session_state.spreadsheet_obj:
                    with st.spinner(f"Writing to '{TODO_WORKSHEET_NAME}' sheet..."):
                        # Important: Export the 'visible_priority_jobs_df' which reflects filters and local actions
                        ...
                        # Add the actual "Next Sched. - Activity" to the export
                        if 'Next Sched. - Activity' in visible_priority_jobs_df.columns:
                            if 'Next Sched. - Activity' not in export_cols: # Avoid duplicates
