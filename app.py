...
def validate_job_pricing(row):
    ...
    """
    # Extract key data - using correct column names and handle string conversion
    material_desc = row.get('Job_Material', '')
    
    # Convert square footage to float, handling potential string values
    sqft_value = row.get('Total_Job_SqFT', 0)
    try:
        total_sqft = float(str(sqft_value).replace(',', '')) if sqft_value else 0
    except (ValueError, TypeError):
        total_sqft = 0
    
    # Convert revenue, handling potential string values with $ and commas
    revenue_value = row.get('Total_Job_Price_', 0) or row.get('Revenue', 0)
    try:
        actual_revenue = float(str(revenue_value).replace('
...
if __name__ == "__main__":
    main()
, '').replace(',', '')) if revenue_value else 0
    except (ValueError, TypeError):
        actual_revenue = 0
    
    # Convert plant cost, handling potential string values
    cost_value = row.get('Phase_Dollars_Plant_Invoice_', 0) or row.get('Cost_From_Plant', 0)
    try:
        actual_plant_cost = float(str(cost_value).replace('
